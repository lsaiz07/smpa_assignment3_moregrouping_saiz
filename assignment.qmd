...
```{r}
impeach%>%
  group_by(for_impeachment, median_income_compared_to_national) %>%
  summarise(total=n()) 
...
impeach %>%
  group_by(for_impeachment) %>%
  summarise(mean_bachelors = mean(pct_bachelors))
...
Im looking for the date anounced by each member of congress who voted "YES" 
...
group_by(for_impeachment, date_announced) %>%
  summarise(total=n()) %>%
  filter(for_impeachment == "YES")%>%
  arrange(desc(total)) %>%
  ()
...
```{r}
  ...
  impeach %>%
  mutate(Date_Year = year(date_announced)) %>%
  mutate(Month_Year = month(date_announced))
